/*
 * WeAct Studio 2.13" E-Paper Text Display Demo
 * Based on SSD1680 driver IC
 *
 * Display: 2.13" Black/White (122x250 pixels)
 * Controller: SSD1680
 * Interface: SPI
 *
...
LOG_MODULE_REGISTER(epaper_213_text, LOG_LEVEL_DBG);
...
/* Display dimensions for 2.13" */
#define EPD_WIDTH  122
#define EPD_HEIGHT 250

/* Frame buffer - round up width to next byte boundary */
#define WIDTH_BYTES ((EPD_WIDTH + 7) / 8)
static uint8_t framebuffer[WIDTH_BYTES * EPD_HEIGHT];
...
/* Initialize display - SSD1680 sequence for 2.13" Black/White */
bool init_display(void)
{
    LOG_INF("Initializing WeAct Studio 2.13\" display (SSD1680)...");
    ...
    /* Driver output control - 250 lines for 2.13" */
    write_cmd(0x01);
    write_data((EPD_HEIGHT - 1) & 0xFF);       // MUX low
    write_data(((EPD_HEIGHT - 1) >> 8) & 0xFF); // MUX high
    ...
    write_data(WIDTH_BYTES - 1);     // X end
...
/* Update display with framebuffer contents - Black/White only */
bool update_display(void)
{
    ...
    /* Write RAM (black/white) - This is a B/W display, no red RAM */
    write_cmd(0x24);
...
{
    LOG_INF("═══════════════════════════════════════");
    LOG_INF("  WeAct Studio 2.13\" E-Paper Text Demo");
    LOG_INF("  Black/White Display (122x250 pixels)");
    LOG_INF("  Driver: SSD1680");
    LOG_INF("═══════════════════════════════════════");
    ...
    draw_string(10, 25, "WeAct 2.13\"", 0, 1);
    draw_string(10, 40, "SSD1680", 0, 1);
    update_display();
    k_msleep(3000);
    ...
    // Title
    draw_string(10, 10, "Zephyr RTOS", 0, 2);

    // Box
    draw_rect(5, 45, 112, 60, 0);

    // Info text
    draw_string(10, 50, "nRF52840-DK", 0, 1);
    draw_string(10, 65, "SSD1680", 0, 1);
    draw_string(10, 80, "122 x 250", 0, 1);
    draw_string(10, 95, "B/W Only", 0, 1);

    // Status
    fill_rect(5, 120, 112, 20, 0);
    ...
    for (int i = 0; i <= 10; i++) {
        clear_buffer(0xFF);
        draw_string(15, 50, "Count:", 0, 2);

        char num[16];
        snprintf(num, sizeof(num), "%d", i);
        draw_string(30, 80, num, 0, 3);
